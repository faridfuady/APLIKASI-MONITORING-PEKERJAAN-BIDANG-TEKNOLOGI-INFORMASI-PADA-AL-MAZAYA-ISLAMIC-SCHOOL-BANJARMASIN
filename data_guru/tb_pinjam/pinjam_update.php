...
$id = $_GET["id"];
$tb_pinjam = queryTampilpinjam("SELECT * FROM tb_pinjam WHERE id = $id")[0];

if(isset($_POST["submit"])) {
  if(ubahpinjam($_POST)>0) {
    echo "<script>alert('Data pinjam Berhasil Diubah');document.location.href = 'pinjam_show.php';</script>";
  } else {
    echo mysqli_error($conn);
    
    // echo"<script>alert('Data pinjam Gagal Diubah');document.location.href = 'pinjam_show.php';</script>";
  }
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
 ...
 <!-- NAVBAR END -->
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Ubah Data Pinjam <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
            <a href="../../index.php">
        ...
        <div class="row">
          ...
          <input type="hidden" name="id" value="<?= $tb_pinjam["id"]; ?>">
          <!-- <input type="hidden" name="gambarLama" value="<?= $tb_pinjam["gambar"]; ?>"> -->
          <div class="mb-3">
                <label for="kd_pinjam" class="form-label">Kode Pinjam</label>
                <input type="text" class="form-control" id="kd_pinjam" name="kd_pinjam" required autocomplete="off"requiredvalue="<?= $tb_pinjam["kd_pinjam"]; ?>">
              </div>
          <div class="mb-3">
                <label for="nip" class="form-label">NIP</label>
                <select class="form-select" name="nip" id="nip">
                  <option selected><?= $tb_pinjam["nip"]; ?></option>
                  <?php 
            ...
            <div class="mb-3">
                  ...
                  <select class="form-select" name="id_brg" id="id_brg">
                    <option selected><?= $tb_pinjam["id_brg"]; ?></option>
                    <?php 
                ...
                <label for="tanggal_pinjam" class="form-label">Tanggal Pinjam</label>
                  <input type="date" class="form-control" id="tanggal_pinjam" name="tanggal_pinjam" required autocomplete="off"requiredvalue="<?= $tb_pinjam["tanggal_pinjam"]; ?>">
              </div>
              <div class="mb-3">
                <label for="keperluan" class="form-label">Keperluan</label>
                <input type="text" class="form-control" id="keperluan" name="keperluan" required autocomplete="off"requiredvalue="<?= $tb_pinjam["keperluan"]; ?>">
              </div>
              <div class="mb-3">
                <label for="status" class="form-label">Status Pinjam</label>
                <select class="form-select" name="status" id="status">
                  <option selected><?= $tb_pinjam["status"]; ?></option>
                      <option value="Dipinjam">Dipinjam</option>
                      <option value="Ready">Ready</option>
                </select>
              </div>
            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Ubah Data pinjam</button>
          </form>
