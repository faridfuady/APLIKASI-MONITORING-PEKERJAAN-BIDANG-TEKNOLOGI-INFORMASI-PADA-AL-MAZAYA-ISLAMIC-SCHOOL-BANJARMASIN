...
$nis = $_GET["nis"];
$mapel = $_GET["mapel"];
$tb_nilai_rapot = queryTampilrapot("SELECT * FROM tb_nilai_rapot WHERE nis = $nis AND mapel = '$mapel'")[0];

if(isset($_POST["submit"])) {
  if(ubahrapot($_POST)>0) {
    echo "<script>alert('Data rapot Berhasil Diubah');document.location.href = 'rapot_show.php';</script>";
  } else {
    echo mysqli_error($conn);
    
    // echo"<script>alert('Data rapot Gagal Diubah');document.location.href = 'rapot_show.php';</script>";
  }
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
    ...
    Data Laporan
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                ...
                <li><a class="dropdown-item" href="../../data/tb_siswa/siswa_print.php">Siswa</a></li>
                 <li><a class="dropdown-item" href="../../data/tb_rapot_siswa_siswa/rapot_siswa_print.php">Rapor Siswa</a></li>
 ...
 <!-- NAVBAR END -->
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Ubah Data rapot <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
            <a href="../../index.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-house-door-fill"></i> Kembali ke Beranda</button>
            </a>
            <a href="rapot_show.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-table"></i> Tabel Data rapot</button>
            </a>
          </div>
        </div>
        <div class="row">
          ...
          <p class="fw-bold"><?= $tb_nilai_rapot["nama"] ?></p>
          <input type="hidden" name="nis" value="<?= $tb_nilai_rapot["nis"]; ?>">
          <input type="hidden" name="mapel" value="<?= $tb_nilai_rapot["mapel"]; ?>">
          
            <div class="mb-3">
              <label for="kelas" class="form-label">Kelas</label>
              <input type="text" class="form-control" id="kelas" name="kelas" autocomplete="off"requiredvalue="<?= $tb_nilai_rapot["kelas"]; ?>"disabled>
            </div>
            <div class="mb-3">
              <label for="mapel" class="form-label">Mata Pelajaran</label>
              <input type="text" class="form-control" id="mapel" name="mapel" autocomplete="off"requiredvalue="<?= $tb_nilai_rapot["mapel"]; ?>"disabled>
            </div>
            <div class="mb-3">
              <label for="tugas" class="form-label">Tugas</label>
              <input type="text" class="form-control" id="tugas" name="tugas" autocomplete="off"requiredvalue="<?= $tb_nilai_rapot["tugas"]; ?>">
            </div>
            <div class="mb-3">
              <label for="uts" class="form-label">UTS</label>
              <input type="text" class="form-control" id="uts" name="uts" autocomplete="off"requiredvalue="<?= $tb_nilai_rapot["uts"]; ?>">
            </div>
            <div class="mb-3">
              <label for="uas" class="form-label">UAS</label>
              <input type="text" class="form-control" id="uas" name="uas" autocomplete="off"requiredvalue="<?= $tb_nilai_rapot["uas"]; ?>">
            </div>

            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Ubah Data rapot</button>
          </form>
