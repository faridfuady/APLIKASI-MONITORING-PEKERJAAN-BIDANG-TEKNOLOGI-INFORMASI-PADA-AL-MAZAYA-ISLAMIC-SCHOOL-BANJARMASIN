...
$id = $_GET["id"];
$tb_kinerja = queryTampilkinerja("SELECT * FROM tb_kinerja WHERE id = $id")[0];

if(isset($_POST["submit"])) {
  if(ubahkinerja($_POST)>0) {
    echo "<script>alert('Data kinerja Berhasil Diubah');document.location.href = 'kinerja_show.php';</script>";
  } else {
    echo mysqli_error($conn);
    
    // echo"<script>alert('Data kinerja Gagal Diubah');document.location.href = 'kinerja_show.php';</script>";
  }
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
 ...
 <!-- NAVBAR END -->
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Ubah Data kinerja <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
            <a href="../../index.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-house-door-fill"></i> Kembali ke Beranda</button>
            </a>
            <a href="kinerja_show.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-table"></i> Tabel Data kinerja</button>
            </a>
          </div>
        </div>
        <div class="row">
          ...
          <input type="hidden" name="id" value="<?= $tb_kinerja["id"]; ?>">
          <div class="mb-3">
              <label for="nip" class="form-label">NIP</label>
              <input type="text" class="form-control" id="nip" name="nip" autocomplete="off"required value="<?= $tb_kinerja["nip"]; ?>">
            </div>
            <div class="mb-3">
              <label for="kedisiplinan" class="form-label">Nilai Kedisiplinan</label>
              <input type="text" class="form-control" id="kedisiplinan" name="kedisiplinan" autocomplete="off"requiredvalue="<?= $tb_kinerja["kedisiplinan"]; ?>">
            </div>
            <div class="mb-3">
              <label for="integritas" class="form-label">Nilai Integritas</label>
              <input type="text" class="form-control" id="integritas" name="integritas" autocomplete="off"requiredvalue="<?= $tb_kinerja["integritas"]; ?>">
            </div>
            <div class="mb-3">
              <label for="tanggung_jwb" class="form-label">Nilai Tanggung Jawab</label>
              <input type="text" class="form-control" id="tanggung_jwb" name="tanggung_jwb" autocomplete="off"requiredvalue="<?= $tb_kinerja["tanggung_jwb"]; ?>">
            </div>
            <div class="mb-3">
              <label for="komunikasi" class="form-label">Nilai Komunikasi</label>
              <input type="text" class="form-control" id="komunikasi" name="komunikasi" autocomplete="off"requiredvalue="<?= $tb_kinerja["komunikasi"]; ?>">
            </div>
            <div class="mb-3">
              <label for="antusiasme" class="form-label">Nilai Antusiasme</label>
              <input type="text" class="form-control" id="antusiasme" name="antusiasme" autocomplete="off"requiredvalue="<?= $tb_kinerja["antusiasme"]; ?>">
            </div>
            <div class="mb-3">
              <label for="tahun" class="form-label">Tahun Kinerja</label>
              <input type="number" class="form-control" id="tahun" name="tahun" autocomplete="off"requiredvalue="<?= $tb_kinerja["tahun"]; ?>">
            </div>

            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Ubah Data kinerja</button>
          </form>
