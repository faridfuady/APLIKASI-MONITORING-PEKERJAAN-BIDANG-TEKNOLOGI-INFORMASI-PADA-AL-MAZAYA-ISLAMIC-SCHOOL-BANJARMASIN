...
if(isset($_POST["submit"])) {
  if(tambahkinerja($_POST)>0) {
    echo "<script>alert('Data kinerja Berhasil Ditambah');document.location.href = 'kinerja_show.php';</script>";


  } 
  else {
    // echo "<script>alert('Data kinerja Gagal Ditambah');document.location.href = 'kinerja_show.php';</script>";
...
<html lang="en">
  <head>
    ...
    <title>Tambah Data kinerja</title>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Tambah Data kinerja <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
            <a href="../../index.php">
        ...
        <div class="row">
          ...
          <div class="mb-3">
                ...
                <select class="form-select" name="nip" id="nip">
                  ...
                  <?php 
                    $sql_nip = mysqli_query($conn, "SELECT * FROM tb_guru ORDER BY nip ASC") or die (mysqli_error($conn));
                    while($data_nip = mysqli_fetch_array($sql_nip)) {
                      echo '
                      <option value="'.$data_nip['nip'].'">
                        '.$data_nip['nip'].' - '.$data_nip['nama'].'
                      </option>
                      ';
                    }
                  ?>
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="kedisiplinan" name="kedisiplinan" autocomplete="off"required>
            </div>
            <div class="mb-3">
              <label for="integritas" class="form-label">Nilai Integritas</label>
              <input type="text" class="form-control" id="integritas" name="integritas" autocomplete="off"required>
            </div>
            <div class="mb-3">
              <label for="tanggung_jwb" class="form-label">Nilai Tanggung Jawab</label>
              <input type="text" class="form-control" id="tanggung_jwb" name="tanggung_jwb" autocomplete="off"required>
            </div>
            <div class="mb-3">
              <label for="komunikasi" class="form-label">Nilai Komunikasi</label>
              <input type="text" class="form-control" id="komunikasi" name="komunikasi" autocomplete="off"required>
            </div>
            <div class="mb-3">
              <label for="antusiasme" class="form-label">Nilai Antusiasme</label>
              <input type="text" class="form-control" id="antusiasme" name="antusiasme" autocomplete="off"required>
            </div>
            <div class="mb-3">
              <label for="tahun" class="form-label">Tahun Kinerja</label>
              <input type="year" class="form-control" id="tahun" name="tahun" autocomplete="off"required>
            </div>
            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Tambah Data kinerja</button>
          </form>
