...
if (isset($_POST['date_filter'])) {
  ...
  $rowsDateFilter = count(queryTampilpinjam("SELECT * FROM tb_barang_pinjam_karyawan WHERE tanggal_pinjam BETWEEN '$startPost%' AND '$endPost%'"));
  ...
  $pinjamDateFilter = queryTampilpinjam("SELECT * FROM tb_barang_pinjam_karyawan WHERE tanggal_pinjam BETWEEN '$startPost%' AND '$endPost%'");
} 
?>  
...
<html lang="en">
  <head>
    ...
    <title>Filter Tanggal Data Pinjam</title>
              ...
              <div class="col">
                  ...
                  <button type="submit" name="date_filter" class="btn btn-info d-inline">Filter</button>
                   ...
                   <?php if (isset($_POST['date_filter'])) { ?>
                    <a href="pinjam-read-date-filter-savepdf.php?start_date=<?= $_POST['start_date']; ?>&end_date=<?= $_POST['end_date']; ?>" target="_blank">
                      <button type="button" class="btn btn-outline-primary"><i class="bi bi-printer-fill"></i> Cetak Data</button>
                    </a>
                      <?php } else { ?>
                        <a href="pinjam-read-date-filter-savepdf.php" target="_blank">
                          <button type="button" class="btn btn-outline-primary"><i class="bi bi-printer-fill"></i> Cetak Data</button>
                        </a>
                   <?php } ?>
            </a>
        ...
        <div class="row">
              <div class="col">
                ...
                <th scope="col">Keperluan</th>
                  ...
                  <?php $i = 1; ?>
                  <?php if (isset($_POST['date_filter'])) : ?>
                    <?php foreach ($pinjamDateFilter as $row) : ?>
                      <tbody>
                        <tr>
                        <th scope="row"><?= $i; ?></th>
                <td><?= $row["nip"]; ?></td>
