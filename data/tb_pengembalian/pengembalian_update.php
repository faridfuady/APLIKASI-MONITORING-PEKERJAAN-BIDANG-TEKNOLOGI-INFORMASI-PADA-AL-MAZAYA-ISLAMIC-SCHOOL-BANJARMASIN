...
$id = $_GET["id"];
$tb_pengembalian = queryTampilpengembalian("SELECT * FROM tb_pengembalian WHERE id = $id")[0];

if(isset($_POST["submit"])) {
  if(ubahpengembalian($_POST)>0) {
    echo "<script>alert('Data pengembalian Berhasil Diubah');document.location.href = 'pengembalian_show.php';</script>";
  } else {
    echo mysqli_error($conn);
    
    // echo"<script>alert('Data pengembalian Gagal Diubah');document.location.href = 'pengembalian_show.php';</script>";
  }
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
 ...
 <!-- NAVBAR END -->
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Ubah Data pengembalian <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
            <a href="../../index.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-house-door-fill"></i> Kembali ke Beranda</button>
            </a>
            <a href="pengembalian_show.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-table"></i> Tabel Data pengembalian</button>
            </a>
          </div>
        </div>
        <div class="row">
          ...
          <input type="hidden" name="id" value="<?= $tb_pengembalian["id"]; ?>">
          <!-- <input type="hidden" name="gambarLama" value="<?= $tb_pengembalian["gambar"]; ?>"> -->
          <div class="mb-3">
                <label for="kd_pinjam" class="form-label">Kode Pinjam</label>
                <select class="form-select" name="kd_pinjam" id="kd_pinjam">
                  <option selected><?= $tb_pengembalian["kd_pinjam"]; ?></option>
                  <?php 
                    $sql_kd_pinjam = mysqli_query($conn, "SELECT * FROM tb_barang_pinjam_karyawan ORDER BY kd_pinjam ASC") or die (mysqli_error($conn));
                    while($data_kd_pinjam = mysqli_fetch_array($sql_kd_pinjam)) {
                      echo '
                      <option value="'.$data_kd_pinjam['kd_pinjam'].'">
                        '.$data_kd_pinjam['kd_pinjam'].' - '.$data_kd_pinjam['nama_brg'].'
                      </option>
                      ';
                    }
                  ?>
                ...
                <label for="tanggal_pengembalian" class="form-label">Tanggal pengembalian</label>
                  <input type="date" class="form-control" id="tanggal_pengembalian" name="tanggal_pengembalian" required autocomplete="off"requiredvalue="<?= $tb_pengembalian["tanggal_pengembalian"]; ?>">
              </div>
              <div class="mb-3">
                <label for="tanggal_pengembalian" class="form-label">Tanggal pengembalian</label>
                  <input type="date" class="form-control" id="tanggal_pengembalian" name="tanggal_pengembalian" required autocomplete="off"requiredvalue="<?= $tb_pengembalian["tanggal_pengembalian"]; ?>">
              </div>
            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Ubah Data pengembalian</button>
          </form>
