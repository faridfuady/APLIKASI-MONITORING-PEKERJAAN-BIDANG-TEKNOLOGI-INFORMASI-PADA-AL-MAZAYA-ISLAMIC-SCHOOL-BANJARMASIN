...
$id = $_GET["id"];
$tb_pemeliharaan = queryTampilpemeliharaan("SELECT * FROM tb_pemeliharaan WHERE id= $id")[0];

if(isset($_POST["submit"])) {
  if(ubahpemeliharaan($_POST)>0) 
  {
    echo "<script>alert('Data pemeliharaan Berhasil Diubah');document.location.href = 'pemeliharaan_show.php';</script>";
  } else {
    echo mysqli_error($conn);
    // echo "<script>alert('Data pemeliharaan Gagal Diubah');document.location.href = 'pemeliharaan_show.php';</script>";
  }
}
...
<html lang="en">
  <head>
    ...
    <title>Ubah Data pemeliharaan</title>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Ubah Data Pemeliharaan <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
        ...
        <div class="row">
          <div class="col">
            <form method="post">
            <input type="hidden" name="id" value="<?= $tb_pemeliharaan["id"]; ?>">
            <div class="mb-3">
                <label for="id_brg" class="form-label">id_brg</label>
                <select class="form-select" name="id_brg" id="id_brg">
                  <option selected><?= $tb_pemeliharaan["id_brg"]; ?></option>
                  <?php 
              ...
              <div class="mb-3">
                ...
                <input type="date" class="form-control" id="tanggal_pemeliharaan" name="tanggal_pemeliharaan" required autocomplete="off"required value="<?= $tb_pemeliharaan["tanggal_pemeliharaan"]; ?>">
              </div>
              <div class="mb-3">
                <label for="keterangan" class="form-label">Keterangan</label>
                <input type="text" class="form-control" id="keterangan" name="keterangan" required autocomplete="off"required value="<?= $tb_pemeliharaan["keterangan"]; ?>">
              </div>
              <div class="mb-3">
                <label for="nm_kryw" class="form-label">Karyawan yang bertugas</label>
                <select class="form-select" name="nm_kryw" id="nm_kryw">
                  <option selected><?= $tb_pemeliharaan["nm_kryw"]; ?></option>
                  <?php 
                ...
                <input type="text" class="form-control" id="catatan" name="catatan" required autocomplete="off"required value="<?= $tb_pemeliharaan["catatan"]; ?>">
              </div>
              </div>
              <button type="submit" name="submit" class="btn btn-success"><i class="bi bi-check-circle-fill"></i> Ubah Data Pengajar</button>
            </form>
          </div>
        </div>
      </div>
