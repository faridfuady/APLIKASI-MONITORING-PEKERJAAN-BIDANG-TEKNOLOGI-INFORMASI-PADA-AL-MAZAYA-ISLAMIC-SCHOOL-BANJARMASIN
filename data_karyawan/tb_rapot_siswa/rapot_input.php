...
if(isset($_POST["submit"])) {
  if(tambahrapot($_POST)>0) {
    echo "<script>alert('Data rapot Berhasil Ditambah');document.location.href = 'rapot_show.php';</script>";


  } 
  else {
    echo "<script>alert('Data rapot Gagal Ditambah');document.location.href = 'rapot_show.php';</script>";
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Tambah Data Rapor <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
        ...
        <div class="row">
          ...
          <div class="mb-3">
                ...
                <select class="form-select" name="nis" id="nis">
                  ...
                  <?php 
                    $sql_nis = mysqli_query($conn, "SELECT * FROM tb_siswa ORDER BY nis ASC") or die (mysqli_error($conn));
                    while($data_nis = mysqli_fetch_array($sql_nis)) {
                      echo '
                      <option value="'.$data_nis['nis'].'">
                        '.$data_nis['nis'].' - '.$data_nis['nama'].'
                      </option>
                      ';
                    }
                  ?>
              ...
              <input type="number" class="form-control" id="kelas" name="kelas" autocomplete="off"required>
            </div>
              <div class="mb-3">
                ...
                <select class="form-select" name="mapel" id="mapel">
                  <option selected></option>
                  <?php 
                    $sql_mapel = mysqli_query($conn, "SELECT * FROM tb_mapel ORDER BY mapel ASC") or die (mysqli_error($conn));
                    while($data_mapel = mysqli_fetch_array($sql_mapel)) {
                      echo '
                      <option value="'.$data_mapel['mapel'].'">
                        '.$data_mapel['mapel'].'
                      </option>
                      ';
                    }
            ...
            <div class="mb-3">
              ...
              <input type="number" class="form-control" id="tugas" name="tugas" autocomplete="off"required>
            </div>
            <div class="mb-3">
              <label for="uts" class="form-label">UTS</label>
              <input type="number" class="form-control" id="uts" name="uts" autocomplete="off"required>
            </div>
            <div class="mb-3">
              <label for="uas" class="form-label">UAS</label>
              <input type="number" class="form-control" id="uas" name="uas" autocomplete="off"required>
            </div>

            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Tambah Data rapot</button>
          </form>
