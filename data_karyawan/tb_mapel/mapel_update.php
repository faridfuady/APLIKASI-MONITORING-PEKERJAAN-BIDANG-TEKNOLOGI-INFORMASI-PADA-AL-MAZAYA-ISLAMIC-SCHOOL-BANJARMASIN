...
require "mapel_functions.php";
$id = $_GET["id"];
$tb_mapel = queryTampilmapel("SELECT * FROM tb_mapel WHERE id = $id")[0];

if(isset($_POST["submit"])) {
  if(ubahmapel($_POST)>0) {
    echo "<script>alert('Data mapel Berhasil Diubah');document.location.href = 'mapel_show.php';</script>";
  } else {
    echo mysqli_error($conn);
    
    // echo"<script>alert('Data mapel Gagal Diubah');document.location.href = 'mapel_show.php';</script>";
  }
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark text-uppercase">
 ...
 <!-- NAVBAR END -->
    ...
    <section id="body">
      <div class="container">
        <h1 class="text-primary-dark mb-5 text-center">Ubah Data Mata Pelajaran <br>   AL-MAZAYA</h1>
        <div class="row mb-3">
          <div class="col-md-9">
            ...
            <a href="mapel_show.php">
              <button type="button" class="btn btn-outline-primary"><i class="bi bi-table"></i> Tabel Data mapel</button>
            </a>
          </div>
        </div>
        <div class="row">
          ...
          <input type="hidden" name="id" value="<?= $tb_mapel["id"]; ?>">
          <div class="mb-3">
              <label for="mapel" class="form-label">Mata Pelajaran</label>
              <input type="text" class="form-control" id="mapel" name="mapel" autocomplete="off"required value="<?= $tb_mapel["mapel"]; ?>">
            </div>
            <div class="mb-3">
              <label for="kkm" class="form-label">KKM</label>
              <input type="text" class="form-control" id="kkm" name="kkm" autocomplete="off"required value="<?= $tb_mapel["kkm"]; ?>">
            </div>
            <div class="mb-3">
                  <label for="guru" class="form-label">Nama Guru</label>
                  <select class="form-select" name="guru" id="guru">
                    <option selected><?= $tb_mapel["guru"]; ?></option>
                    <?php 
                      $sql_nama = mysqli_query($conn, "SELECT * FROM tb_guru ORDER BY nama ASC") or die (mysqli_error($conn));
            ...
            <button type="submit" name="submit" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i> Ubah Data mapel</button>
          </form>
